...
void stretching(unsigned char*);
void automaticdetection(int*, int*, int*);
...
int menu(){
    ...
    do{
        printf("\n\nIndica la tua scelta <1/2/3/4/5/6>: ");
        scanf("%d", &s);
    }while(s<1 || s>6);
...
void generatehist(unsigned char* histo, int* o, int o_max, int l){
    ...
    for(i=0; i<DATA_DIM*DATA_DIM; i++){
        *(histo+i)=10; //sfondo grigio-scuro
	}
	//NOTA:le "i" vanno dal basso verso l'alto, le j da sx verso dx
    for(k=0; k<l; k++){
        for(j=start; j<start+amp; j++){
            for(i=0; i<((*(o+k)*(DATA_DIM-1-20))/o_max); i++){
                *(histo+(i+20)*DATA_DIM+j)=200;
            }
        }
        start+=amp;
    }
    for(j=0; j<GRAYSCALE; j++){
        if(j%25==0){
            if(j%50==0){
                if(j%100==0){
                    for(i=0; i<17; i++)
                        *(histo+i*DATA_DIM+j*(DATA_DIM/GRAYSCALE))=255;
                }
                for(i=0; i<9; i++)
                    *(histo+i*DATA_DIM+j*(DATA_DIM/GRAYSCALE))=255;
            }
...
void automaticdetection(int* occ, int* c, int* d){
    int noise = 300;
    char r;
    int j;

    printf("Desideri scegliere il numero minimo di pixel appartenenti ad un livello affinch%c il livello non sia considerato rumore? ", 138);
    do{
        printf("\n\n<y/n>: ");
        scanf("\n%c", &r);
    }while(r!='y' && r!='n');
    ...
    if(r=='y'){
        ...
        do{
            printf("\n\nValore (0-%d): ", (DATA_DIM*DATA_DIM)/GRAYSCALE);
            scanf("\n%d", &noise);
        }while(noise<0 || noise > (DATA_DIM*DATA_DIM)/GRAYSCALE);
    }
