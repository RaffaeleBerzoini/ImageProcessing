...
int verifica(char s[], char d[], int m){
    ...
    if(m==LOAD){
        ...
        a = loadBMP(d, &img);
        if(a==0){
            a = checkbmp(d); //se l'header è corretto l'errore della mancanza del formato corretto nel percorso viene ignorato
        }
    }
...
int checkbmp(char s[]){ //se carico un file che non ha nessun formato ma che è stato salvato tramite saveBMP la funzione LOAD non individua l'errore
...
void clustering(unsigned char* img, int h, int w){
    ...
    int i, j, k, diff, clusters; //numero di "parti" nelle quali verrà suddivisa l'immagine
    int min, tmp;
    int* clust_val;
